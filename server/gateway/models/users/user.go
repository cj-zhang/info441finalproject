...
func (nu *NewUser) Validate() error {
	...
	if len(nu.Password) < 6 {
		return fmt.Errorf("password must be at least 6 chars")
...
func (nu *NewUser) ToUser() (*User, error) {
	...
	emailStr := fmt.Sprintf("%x", emailHash)
	result.PhotoURL = gravatarBasePhotoURL + emailStr

	result.SetPassword(nu.Password)
...
func (u *User) ApplyUpdates(updates *Updates) error {
	...
	if updates == nil {
		return fmt.Errorf("no updates given to apply")
	}
