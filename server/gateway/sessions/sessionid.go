...
func NewSessionID(signingKey string) (SessionID, error) {
	if len(signingKey) == 0 {
		return InvalidSessionID, errors.New("signing key not found")
	}

	fullSlice := make([]byte, signedLength)
	idSlice := make([]byte, idLength)
	_, err := rand.Read(idSlice) // load idLength of bytes with random bytes
	if err != nil {
		return InvalidSessionID, err
	}
	copy(fullSlice, idSlice) // copy into the final slice

	h := hmac.New(sha256.New, []byte(signingKey)) // create the hash
	h.Write(idSlice)                              // write the idSlice
	signature := h.Sum(nil)
	copy(fullSlice[idLength:], signature) // copy into the latter half of the big slice

	result := base64.URLEncoding.EncodeToString(fullSlice)
...
func ValidateID(id string, signingKey string) (SessionID, error) {
	...
	decodedString, err := base64.URLEncoding.DecodeString(id)
	if err != nil {
		return InvalidSessionID, err
	}

	h := hmac.New(sha256.New, []byte(signingKey))
	h.Write(decodedString[0:idLength])
	signature := h.Sum(nil)
	if hmac.Equal(signature, decodedString[idLength:]) {
